...
function PromptCard({post,handleTagClick,handleEdit,handleDelete }) {
  ...
  const router=useRouter();
    ...
    const userProfile=()=>{
      if(session?.user.id === post.creator._id) return router.push(`/profile?id=${post.creator.id}`)
      router.push(`/profile/${post.creator._id}?name=${post.creator.username}`)
    }
  return (
    <div className="prompt_card">
        <div className="flex justify-between items-start gap-5">
            <div className=" flex flex-1 flex-justify items-center gap-3 cursor-pointer">
                <Image src={post.creator.image} alt='user' width={40} height={40} className="rounded-full object-contain" onClick={userProfile}/>
